...
namespace JX.ADO
{
	...
	/// 数据库表：Files 的仓储实现类.
	/// </summary>
	public partial class FilesRepositoryADO : IFilesRepositoryADO
	{
		#region 数据上下文
		/// <summary>
		/// 数据上下文
		...
		/// </summary>
		/// <param name="DB"></param>
		public FilesRepositoryADO(IDBOperator DB)
		{
			_DB = DB;
		}
		#endregion
		...
		{
			string strSQL = "select " + statistic + " from Files where 1=1 ";
		...
		{
			return await Task.Run(() => _DB.GetMaxID("Files", "ID"));
		}
		...
		/// <param name="entity">实体模型</param>
		/// <returns></returns>
		public virtual async Task<bool> AddAsync(FilesEntity entity)
		{
			if(entity.ID <= 0) entity.ID=GetNewID();
			Dictionary<string, object> dict = new Dictionary<string, object>();
			GetParameters(entity, dict);
		...
		/// <param name="entity">实体模型</param>
		/// <returns></returns>
		public virtual int Insert(FilesEntity entity)
		{
			if(entity.ID <= 0) entity.ID=GetNewID();			
			Dictionary<string, object> dict = new Dictionary<string, object>();
			GetParameters(entity,dict);
		...
		/// <returns></returns>
		public virtual bool AddOrUpdate(FilesEntity entity, bool IsSave)
		{
		...
		/// <param name="IsSave">是否增加</param>
		/// <returns></returns>
		public virtual async Task<bool> AddOrUpdateAsync(FilesEntity entity, bool IsSave)
		{
		...
		{
			string strSQL = "delete from Files where " +
		...
		{
			string strSQL = "delete from Files where 1=1 " + strWhere;
			return _DB.ExeSQLResult(strSQL, dict);
		}
		/// <summary>
		/// 删除一条或多条记录（异步方式）
		...
		/// <param name="entity">实体模型</param>
		/// <returns></returns>
		public virtual bool Update(FilesEntity entity)
		{
			Dictionary<string, object> dict = new Dictionary<string, object>();
			GetParameters(entity, dict);
		...
		public virtual bool Update(string strColumns, Dictionary<string, object> dictColumns = null, string strWhere = "", Dictionary<string, object> dictWhere = null)
		{
			...
			string strSQL = "Update Files SET " + strColumns + " where 1=1 " + strWhere;
		...
		/// </summary>
		/// <returns></returns>
		public FilesEntity GetEntity(System.Int32 id)
		{
		...
		/// </summary>
		/// <returns></returns>
		public async Task<FilesEntity> GetEntityAsync(System.Int32 id)
		{
			string strCondition = string.Empty;
			strCondition += " and ID = @ID";
		...
		/// <param name="dict">参数的名/值集合</param>
		/// <returns></returns>
		public virtual IList<FilesEntity> GetEntityList(string strWhere="", Dictionary<string, object> dict = null)
		{
			IList<FilesEntity> list = new List<FilesEntity>();
			string strSQL = "select * from Files where 1=1 ";
			if(!string.IsNullOrEmpty(strWhere))
			{
				strSQL += strWhere;
			}
		...
		/// <param name="dict">参数的名/值集合</param>
		/// <returns></returns>
		public virtual async Task<IList<FilesEntity>> GetEntityListAsync(string strWhere = "", Dictionary<string, object> dict = null)
		{
			IList<FilesEntity> list = new List<FilesEntity>();
			string strSQL = "select * from Files where 1=1 ";
			if (!string.IsNullOrEmpty(strWhere))
		...
		{
			string strSQL = "select " + strExtended + " from Files where 1=1 ";
			if(!string.IsNullOrEmpty(strWhere))
			{
				strSQL += strWhere;
			}
		...
		/// <returns></returns>
		public IList<FilesEntity> GetList(int startRowIndexId, int maxNumberRows,string Filter, out int Total)
		{
			Total = 0;
		...
		/// <param name="Total">输出参数：查询总数</param>
		/// <returns></returns>
		public IList<FilesEntity> GetListBySortColumn(int startRowIndexId, int maxNumberRows,string Filter, out int Total)
		{
			Total = 0;
		...
		/// <param name="Total">输出参数：查询总数</param>
		/// <returns></returns>
		public IList<FilesEntity> GetListBySortColumn(int startRowIndexId, int maxNumberRows,string Sorts,string Filter, out int Total)
		{
			Total = 0;
